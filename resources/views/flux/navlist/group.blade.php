...
<ui-disclosure {{ $attributes->class('group/disclosure') }} @if ($expanded === true) open @endif
    ...
    <button type="button"
        class="group/disclosure-button mb-[2px] flex h-10 w-full items-center rounded-lg text-zinc-500 hover:bg-zinc-800/5 hover:text-zinc-800 lg:h-8 darky:text-white/80 darky:hover:bg-white/[7%] darky:hover:text-white">
        <div class="pl-3 pr-4">
    ...
    <div class="relative hidden space-y-[2px] pl-7 data-open:block" @if ($expanded === true) data-open @endif>
        <div class="absolute inset-y-[3px] left-0 ml-4 w-px bg-zinc-200 darky:bg-white/30"></div>
